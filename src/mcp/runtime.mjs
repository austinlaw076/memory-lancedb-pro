...
// Import from the plugin's TypeScript source using jiti
const importPlugin = (path) => jiti(import.meta.url)(`../../${path}`);

let parsePluginConfig, AccessTracker, createEmbedder, getVectorDimensions, 
    createGraphitiBridge, createGraphitiSyncService, isNoise, 
    getDisplayCategoryTag, createRetriever, DEFAULT_RETRIEVAL_CONFIG, 
    MemoryScopeManager, MemoryStore, validateStoragePath;

// Lazy load plugin modules
async function loadPluginModules() {
  if (parsePluginConfig) return;
  
  const plugin = await importPlugin("index.js");
  parsePluginConfig = plugin.parsePluginConfig;
  
  const storeMod = await importPlugin("src/store.js");
  MemoryStore = storeMod.MemoryStore;
  validateStoragePath = storeMod.validateStoragePath;
  
  const embedderMod = await importPlugin("src/embedder.js");
  createEmbedder = embedderMod.createEmbedder;
  getVectorDimensions = embedderMod.getVectorDimensions;
  
  const retrieverMod = await importPlugin("src/retriever.js");
  createRetriever = retrieverMod.createRetriever;
  DEFAULT_RETRIEVAL_CONFIG = retrieverMod.DEFAULT_RETRIEVAL_CONFIG;
  
  const scopesMod = await importPlugin("src/scopes.js");
  MemoryScopeManager = scopesMod.MemoryScopeManager;
  
  const accessMod = await importPlugin("src/access-tracker.js");
  AccessTracker = accessMod.AccessTracker;
  
  const noiseMod = await importPlugin("src/noise-filter.js");
  isNoise = noiseMod.isNoise;
  
  const reflectionMod = await importPlugin("src/reflection-metadata.js");
  getDisplayCategoryTag = reflectionMod.getDisplayCategoryTag;
  
  const bridgeMod = await importPlugin("src/graphiti/bridge.js");
  createGraphitiBridge = bridgeMod.createGraphitiBridge;
  
  const syncMod = await importPlugin("src/graphiti/sync.js");
  createGraphitiSyncService = syncMod.createGraphitiSyncService;
}

export async function createMemoryMcpRuntime(options = {}) {
  ...
  const { createFileLogger } = await import("./file-logger.mjs");
  ...
  const accessTracker = new AccessTracker({ store, logger,
  ...
  const graphitiBridge = parsedPluginConfig.graphiti?.enabled
    ? createGraphitiBridge({
        ...
        logger: {
          warn: (message) => logger.warn("graphiti_warn", { message }),
        },
      })
    : undefined;
  ...
  const graphitiSync = graphitiBridge
    ? createGraphitiSyncService({
        ...
        store,
        logger: {
          warn: (message) => logger.warn("graphiti_sync_warn", { message }),
        },
      })
    : undefined;
  ...
  function resolveScopeFilter(requestedScope) {
    if (requestedScope) {
  ...
  function resolveTargetScope(requestedScope) {
  ...
  async function toolForget(args) {
    const traceId = trace();
    const startedAt = Date.now();
  ...
  async function toolGraphRecall(args) {
    const traceId = trace();
    const startedAt = Date.now();
    const query = String(args.query || "").trim();
    const scope = typeof args.scope === "string" ? args.scope.trim() : undefined;
...
function sanitizeRetrievalResult(result) {
  return {
    score: result.score,
...
function resolveDbPath(configPath, dbPath) {
